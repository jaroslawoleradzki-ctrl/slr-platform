...
export interface MultiReviewerMetrics { incomplete: number; agreement: number; conflict: number; agreement_rate: number | null; }
...
export const screeningApi = {
  ...
  getConflicts(projectId: string, stage: 'title_abstract' | 'full_text', status?: string | null, offset = 0, limit = 50, viewerReviewerId?: string | null) { return request<{ total: number; offset: number; limit: number; items: ScreeningConflict[] }>(`/projects/${projectId}/screening/conflicts?${query({ stage, status, offset, limit, viewer_reviewer_id: viewerReviewerId })}`); },
  getConflictMetrics(projectId: string, stage: 'title_abstract' | 'full_text') { return request<{ incomplete: number; agreement: number; conflict: number; agreement_rate: number | null }>(`/projects/${projectId}/screening/conflict-metrics?${query({ stage })}`); },
};
