...
import { ApiDuplicateGroup, DeduplicationSummary } from '../../types';
...
export const DeduplicationSummaryCard: React.FC<DeduplicationSummaryCardProps> = ({ summary, groups, hasRun = false }) => {
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
            ...
            >
              <span style={{ fontSize: '0.75rem', color: 'var(--status-warning-text)', fontWeight: 700 }}>
                Oczekujące (PENDING)
              </span>
              <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '2px' }}>
                {pendingGroups.toLocaleString()}
              </div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>Wymagają oceny</span>
            </div>
            ...
            >
              <span style={{ fontSize: '0.75rem', color: 'var(--status-success-text)', fontWeight: 700 }}>
                Zatwierdzone (APPROVE)
              </span>
              <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '2px' }}>
                {approvedGroups.toLocaleString()}
              </div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>Potwierdzone duplikaty</span>
            </div>
            ...
            >
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Odrzucone (REJECT)</span>
              <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '2px' }}>
                {rejectedGroups.toLocaleString()}
              </div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>Potwierdzone unikalne</span>
            </div>
          </div>
        </div>
      </Card>
  ...
  return (
    <Card
      ...
      action={
        legacySummary.candidateGroupsPendingUserReview > 0 ? (
          <Badge variant="pending_action" icon={<ShieldAlert size={12} />}>
            Duplicate groups awaiting human review ({legacySummary.candidateGroupsPendingUserReview})
          </Badge>
        ) : (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        >
          ...
          <span>
            <strong>Zasada domenowa backendu:</strong> Kandydaci na duplikaty wykrywani są wyłącznie na podstawie silnych identyfikatorów (<strong>DOI</strong>, <strong>PMID</strong>, <strong>OpenAlex ID</strong>). Wykryte grupy <strong>nie są scalane automatycznie</strong> przez backend. Decyzje badacza zapisywane są trwale w bazie SQLite.
          </span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
          ...
          >
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)', marginTop: '2px' }}>
              {legacySummary.recordsBeforeDedup.toLocaleString()}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--status-success-text)', marginTop: '2px' }}>
              {legacySummary.identifierLinkedGroupsCount.toLocaleString()}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)', marginTop: '2px' }}>
              {legacySummary.recordsAfterResultMerger.toLocaleString()}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '2px' }}>
              {legacySummary.candidateGroupsPendingUserReview} grup
            </div>
