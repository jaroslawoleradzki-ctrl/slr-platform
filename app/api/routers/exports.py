...
def export_bibtex(
    ...
    raw_content = render_bibtex(publications)
    d8_line = f"%% Generated by SLR Platform {version} for project {project_id} at {generated_at}"
    content = f"{raw_content}\n{d8_line}\n" if raw_content else f"{d8_line}\n"
...
def export_ris(
    ...
    version = get_app_version()
    generated_at = datetime.now(timezone.utc).isoformat()
    raw_content = render_ris(publications)
    d8_line = f"%% Generated by SLR Platform {version} for project {project_id} at {generated_at}"
    content = f"{raw_content}\n{d8_line}\n" if raw_content else f"{d8_line}\n"
    return _attachment_response(
        content,
