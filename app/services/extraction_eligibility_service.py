...
@runtime_checkable
class QualityAssessmentCompletionReader(Protocol):
    """Read-only QA completion boundary consumed by Data Extraction."""
...
class RepositoryQualityAssessmentCompletionReader:
    """Repository-backed QA completion reader with no workflow or write access."""

    def __init__(
        self,
        config_repo: ProjectQualityAssessmentConfigurationRepository | None = None,
        assessment_repo: QualityAssessmentRepository | None = None,
    ) -> None:
        self._config_repo = (
            config_repo or default_project_quality_assessment_configuration_repository()
        )
        self._assessment_repo = assessment_repo or default_quality_assessment_repository()

    def is_configured(self, project_id: str) -> bool:
        return self._config_repo.get_configuration(project_id) is not None
    ...
    def has_completed_assessment(
        ...
        if not reviewer_id.strip():
            return False
        return (
            self._assessment_repo.get_latest_assessment(
                project_id, publication_id, reviewer_id.strip()
            )
            is not None
        )
...
class ExtractionEligibilityService:
    ...
    def __init__(
        ...
        qa_completion_reader: QualityAssessmentCompletionReader | None = None,
    ) -> None:
        ...
        self._qa_completion_reader = (
            qa_completion_reader or RepositoryQualityAssessmentCompletionReader()
        )
    ...
    def evaluate_publication(
        ...
        if self._qa_completion_reader.is_configured(project_id):
            if not self._qa_completion_reader.has_completed_assessment(
