...
def test_normalization_is_project_scoped_and_returns_real_summary(tmp_path: Path) -> None:
    ...
    response = client.post("/projects/lean_energy/normalization")
    ...
    assert client.get("/projects/lean_energy/normalization").json() == body
...
def test_empty_project_is_a_real_zero_record_normalization(tmp_path: Path) -> None:
    ...
    response = client.post("/projects/ai_architecture/normalization")
...
def test_unknown_project_and_repeat_are_safe(tmp_path: Path) -> None:
    ...
    assert client.post("/projects/missing/normalization").status_code == 404
    first = client.post("/projects/lean_energy/normalization").json()
    second = client.post("/projects/lean_energy/normalization").json()
...
def test_saved_result_survives_reopening_sqlite_repository(tmp_path: Path) -> None:
    ...
    created = client.post("/projects/lean_energy/normalization").json()
    ...
    assert restarted_client.get("/projects/lean_energy/normalization").json() == created
...
def test_second_run_replaces_latest_project_result(tmp_path: Path) -> None:
    ...
    assert client.get("/projects/lean_energy/normalization").json() == second
