...
class TestF11CrossProjectIsolation:
    ...
    def test_api_creation_and_retrieval_are_project_scoped(self, client):
        ...
        resp_a = test_client.post(f"/projects/{proj_a}/synthesis/snapshots", json={"actor": "researcher-1"})
        assert resp_a.status_code == 201
        resp_b = test_client.post(f"/projects/{proj_b}/synthesis/snapshots", json={"actor": "researcher-1"})
        assert resp_b.status_code == 201

        detail_a = test_client.get(f"/projects/{proj_a}/synthesis/snapshots/1")
        detail_b = test_client.get(f"/projects/{proj_b}/synthesis/snapshots/1")
...
class TestF12NoMutationPath:
    ...
    def test_api_rejects_put_patch_delete(self, client):
        ...
        _seed_project(db_path, proj_id)
        test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "researcher-1"})

        assert test_client.put(f"/projects/{proj_id}/synthesis/snapshots/1", json={}).status_code == 405
        assert test_client.patch(f"/projects/{proj_id}/synthesis/snapshots/1", json={}).status_code == 405
        assert test_client.delete(f"/projects/{proj_id}/synthesis/snapshots/1").status_code == 405
