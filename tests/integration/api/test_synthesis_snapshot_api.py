...
def test_d1_create_snapshot(client):
    ...
    res = test_client.post(
        f"/projects/{proj_id}/synthesis/snapshots",
...
def test_d2_create_snapshot_missing_project(client):
    ...
    res = test_client.post("/projects/missing/synthesis/snapshots", json={"actor": "r"})
...
def test_d3_create_snapshot_validation_error(client):
    ...
    res = test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "  "})
...
def test_d4_list_snapshots_ordered_by_version(client):
    ...
    test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "r"})
    test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "r"})
    test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "r"})

    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots")
...
def test_d5_get_snapshot_by_version(client):
    ...
    created = test_client.post(f"/projects/{proj_id}/synthesis/snapshots", json={"actor": "r"}).json()

    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots/{created['version']}")
    assert res.status_code == 200
...
def test_d6_get_snapshot_missing_version(client):
    ...
    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots/99")
    assert res.status_code == 404
...
def test_d7_json_export_contract(client):
    ...
    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots/1/export?format=json")
    assert res.status_code == 200
    data = res.json()
...
def test_d8_csv_export_contract(client):
    ...
    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots/1/export?format=csv")
    assert res.status_code == 200
    data = res.json()
...
def test_d9_unsupported_export_format(client):
    ...
    res = test_client.get(f"/projects/{proj_id}/synthesis/snapshots/1/export?format=xml")
    assert res.status_code == 400
...
def test_d10_project_isolation(client):
    ...
    created_a = test_client.post(f"/projects/{proj_a}/synthesis/snapshots", json={"actor": "r"}).json()
    test_client.post(f"/projects/{proj_b}/synthesis/snapshots", json={"actor": "r"}).json()

    # Project B's version 1 is its own snapshot, never project A's.
    res = test_client.get(f"/projects/{proj_b}/synthesis/snapshots/1")
    assert res.status_code == 200
    ...
    res = test_client.get(f"/projects/{proj_a}/synthesis/snapshots/{created_a['version']}")
    assert res.status_code == 200
