...
- **7.3 — Screening Configuration GUI** ⬜ — Next implementation increment. Graphical management interface for screening criteria: listing criteria, create/edit/deactivate actions, inclusion vs. exclusion selection, stage targeting (`TITLE_ABSTRACT` / `FULL_TEXT` / `BOTH`), required/optional toggles, reordering controls, description/instruction fields, backend API persistence integration, and full loading, empty, error, and validation state handling. Zero hardcoded criteria.
