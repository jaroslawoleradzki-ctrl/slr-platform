...
- **7.5 — Title & Abstract Screening** ⬜ — Next implementation increment. Queue management for post-deduplication Working Collection records, presentation of title, abstract, and bibliographic metadata, evaluation against `TITLE_ABSTRACT` and `BOTH` criteria, criterion-level assessment, `INCLUDE` / `EXCLUDE` / `UNCERTAIN` decision recording with rationale, previous/next queue navigation, save & resume capabilities, queue filtering, progress tracking, and full GUI & backend integration. No AI screening decisions.
