...
Key architectural decisions:
- **Project-Scoped Persistence & Isolation**: All repository queries (`SELECT`, `UPDATE`, `DEACTIVATE`) enforce `WHERE project_id = ? AND criterion_id = ?`. Cross-project access attempts via URL paths return `CriterionNotFoundError` translated to `404 Not Found`, hiding non-existent or foreign criteria.
- **Identity & Project Ownership Immutability**: `criterion_id` is a stable UUID generated by the backend domain logic. `project_id` and `criterion_id` cannot be altered or transferred through `PUT` update payloads.
- **Soft Deactivation Lifecycle (No Physical Delete)**: Physical `DELETE` operations are intentionally excluded. `PATCH /projects/{project_id}/screening/criteria/{criterion_id}/deactivate` sets `is_active=False` while preserving the full criterion record. This design decision preserves the criterion's stable identity (`criterion_id`), avoids destroying persistent records, enables future historical references by `ScreeningDecision` (Phase 7.4), and prepares the architecture for subsequent criterion versioning and snapshots.
- **Non-Unique Display Order with Deterministic Tie-Breaking**: `display_order` is non-unique (multiple criteria can share the same display index). Repositories and API list queries enforce a deterministic ordering contract via `ORDER BY display_order ASC, criterion_id ASC`.
- **Pure Schema without Redundant Timestamps**: Migration `0007_screening_criteria.sql` maps the exact domain fields of `ScreeningCriterion` without adding unrequested `created_at` or `updated_at` timestamps to the domain model or database table.
...
- **Phase 6.8 Status Bounds**:
  ...
  - `6.8.2 Provider-Specific Query Rendering`: ✅ Completed in v0.2.6 (`OpenAlexQueryRenderer`, `CrossrefQueryRenderer`, `RenderedQuery` lossy tracking)
